...
extern TRC tone_curve; //raw tone curve(gamma) R、G、B、W、target
extern double targetx, targety; //目標色域座標
extern vector<double> ref_white; //參考白
extern MATRIX matrix_raw;

extern Test_LUT test_lut[]; //校正時的測試LUT 
extern Calibrated_LUT calibrated_lut[]; //校正時最優解LUT
extern RGBColor one_d_lut[]; //實際傳送的LUT
extern RGBColor ca_lut[]; //紀錄色溫校正後的LUT 供 tone curve 、dicom 運算時使用
extern RGBColor dicom_lut[];

double tone_curve_raw;
...
void CCa200SampleDlg::Write_PIPE_GAMMA_LUT(double gamma) {
	CFile fout;
	CString filename;
	if (gamma < 1) {
		filename.Format(".\\data\\panel_attribute\\en_gamma.csv");
	}
	else {
		filename.Format(".\\data\\panel_attribute\\de_gamma.csv");
	}
	fout.Open(filename, CFile::modeCreate | CFile::modeWrite);
	fout.Write("i,R,G,B\n", 8);//
	...
	double w;
	double o;
	for (uint16_t LUT_index = 0; LUT_index < LUT_array_size; LUT_index++) {
		gamma_rgb = panel_1d_lut[LUT_index];
		char str[255];
		sprintf_s(str, "%d,", LUT_index);
		fout.Write(str, strlen(str));
		sprintf_s(str, "%d,", gamma_rgb.Red);
		fout.Write(str, strlen(str));
		sprintf_s(str, "%d,", gamma_rgb.Green);
		fout.Write(str, strlen(str));
		sprintf_s(str, "%d\n", gamma_rgb.Blue);
		fout.Write(str, strlen(str));
	}
...
void CCa200SampleDlg::Init_LUT() {
	...
	for (uint16_t i = 0; i < LUT_array_size; i++) {
		one_d_lut[i] = RGBColor{ i,i,i };
 		panel_1d_lut[i] = RGBColor{ i,i,i };
	}
}

void CCa200SampleDlg::Send_Get(vector<vector<int>> send_index, int16_t nRet, int32_t LinkNo, vector<vector<double> >& data) {
	// CA-SDK
	int Ri;
	int Gi;
	int Bi;
	...
	for (int i = 0; i < send_index.size(); i++) {
		...
		Gi = send_index[i][1];
		Bi = send_index[i][2];
		m_str232.Format("Message send: %d,%d,%d", one_d_lut[Ri].Red, one_d_lut[Gi].Green, one_d_lut[Bi].Blue);
		SetDlgItemText(IDC_STATIC_232, m_str232);

		nRet = Generate_RGB_Pattern(LinkNo, one_d_lut[Ri].Red, one_d_lut[Gi].Green, one_d_lut[Bi].Blue);//使用內建LUT
		Sleep(300);
		...
		data.push_back({ (double)one_d_lut[Ri].Red,(double)one_d_lut[Gi].Green,(double)one_d_lut[Bi].Blue,fX,fY,fZ,fLv,fx,fy,fz,(double)lT,fduv }); //儲存R,G,B,X,Y,Z,Lv,sx,sy,sz,lT,duv
...
vector<double>CCa200SampleDlg::Measure_MaxLv(RGBColor RGB) {
	...
	Send_Get(send_index, nRet, LinkNo, data);
...
void CCa200SampleDlg::Measure_Raw() {
	...
	tone_curve.r = calculate_trc(lv, std_lv);
	lv.clear();
	...
	tone_curve.g = calculate_trc(lv, std_lv);
	lv.clear();
	...
	tone_curve.b = calculate_trc(lv, std_lv);
	lv.clear();
	...
	tone_curve.w = calculate_trc(lv, std_lv);

	ref_white = XYZ_R;//參考白

	m_raw_trc_r.Format("%.2f", tone_curve.r);
	m_raw_trc_g.Format("%.2f", tone_curve.g);
	m_raw_trc_b.Format("%.2f", tone_curve.b);
	m_raw_trc_w.Format("%.2f", tone_curve.w);
...
void CCa200SampleDlg::CCT_Trans_LUT(MATRIX matrix_cct, PRGBColor ptr_ca_lut, PRGBColor ptr_cct_lut) {
	uint16_t R, G, B;
	vector<float>xyz = { matrix_cct[0][0], matrix_cct[1][1], matrix_cct[2][2] };
	for (int i = 0; i < LUT_array_size; i++) {
		R = ptr_ca_lut->Red * xyz[0];
		G = ptr_ca_lut->Green * xyz[1];
		B = ptr_ca_lut->Blue * xyz[2];
		ptr_cct_lut->Red = R;
		ptr_cct_lut->Green = G;
		ptr_cct_lut->Blue = B;
		ptr_ca_lut++;
...
void CCa200SampleDlg::TRC_Trans_LUT(RGBColor lut[], PRGBColor ptr) {
	uint16_t R, G, B;
	for (int i = 0; i < 1024; i++) {
		R = pow((double)lut[i].Red / (double)lut[1023].Red, tone_curve.target / tone_curve_raw) * (double)lut[1023].Red;
		G = pow((double)lut[i].Green / (double)lut[1023].Green, tone_curve.target / tone_curve_raw) * (double)lut[1023].Green;
		B = pow((double)lut[i].Blue / (double)lut[1023].Blue, tone_curve.target / tone_curve_raw) * (double)lut[1023].Blue;
		ptr->Red = R;
		ptr->Green = G;
		ptr->Blue = B;
		ptr++;
...
void CCa200SampleDlg::Update_Panel_1d_Lut(RGBColor lut[]) {
	for (int i = 0; i < LUT_array_size; i++) {
		panel_1d_lut[i] = lut[i];
	}
}

void CCa200SampleDlg::Update_Panel_1d_Lut_TRC(double gamma) {
	RGBColor gamma_rgb;
	uint16_t v;
	double w;
	double o;
...
void CCa200SampleDlg::Init_Panel_Matrix_1D_LUT_GAMMA() {
	...
	SetDlgItemText(IDC_STATIC_xy, xy_str);
	Update_Gamut_Matrix(matrix_raw);
	UpdateMatrixRegisters();
	Update_Panel_1d_Lut_TRC(1);
...
void CCa200SampleDlg::Update_Gamut_Matrix(MATRIX matrix) {
	UpdateData(TRUE);
...
void CCa200SampleDlg::UpdateMatrixRegisters()
{
	...
	static CString s;
	s.Format("%s", gamut_target);
	SetDlgItemText(IDC_STATIC_Select_Gamut ,s);
...
void CCa200SampleDlg::XferLut2Mcu() {
	...
	for (LUT_index = 0; LUT_index < LUT_array_size; LUT_index++)
	{
		...
		SetDlgItemText(IDC_STATIC_xy, xy_str);
		nError = Xfer_LUTable1D_Element(LinkNo, LUT_index, &(panel_1d_lut[LUT_index]));
		if (nError != ERROR_SCALER_SUCCESS) {
			break;
		}
	}
...
void CCa200SampleDlg::UpdateLutBuffer2FPGA()
{
	...
	CString s;
	s.Format("%d", cct_target);
	SetDlgItemText(IDC_STATIC_Select_CCT, s);
}
...
void CCa200SampleDlg:: CCT_Calibration() {
	Init_LUT();
	vector<float>xy = CCT_to_xy(cct_target);
	targetx = xy[0];
	targety = xy[1];
	xy_str.Format("Calibrating %s, %dK, x: %f, y: %f", gamut_target, cct_target, targetx, targety);
	SetDlgItemText(IDC_STATIC_xy, xy_str);
	//將要傳送的灰階序號更新為測試用的16點灰階階數
	send_index.clear();
	for (int k = 0; k < interval; k++) {
		send_index.push_back({ (k + 1) * step - 1,  (k + 1) * step - 1 ,  (k + 1) * step - 1 });
		test_lut[k].rgbColor.Red = (k + 1) * step - 1;
		test_lut[k].rgbColor.Green = (k + 1) * step - 1;
		test_lut[k].rgbColor.Blue = (k + 1) * step - 1;
	}
	//cct
	int c = 0;
	while (true) {
		if (c == 20) { break; }
			...
			Cal_CCT_LUT(); //產生cct_lut
			Send_Get(send_index, nRet, LinkNo, data); //測量
			Read_Test_Data(data); //讀取結果
			Analysis(); //分析結果
		}
		c++;
	}
	Build_CCT_LUT(); //建置 0~1023 完整 LUT(one_d_lut)
	Init_LUT();
}

void CCa200SampleDlg::Meaure_Color(CString gamut, int cct, double trc) {
	Check_State();
	send_index.clear();
	...
	if (trc == 0.0) {
		str.Format(".\\data\\lut\\all_lut_verify\\%s\\lut_%s_%d_dicom.csv", gamut, gamut, cct);
	}
	else {
		str.Format(".\\data\\lut\\all_lut_verify\\%s\\lut_%s_%d_%.2f.csv", gamut, gamut, cct, trc);
	}
...
void CCa200SampleDlg:: Colour_Calibrate() {
	...
	CString de_gamma_lut_filename;
	CString en_gamma_lut_filename;
	CString gamut_65_lut_filename;
	CString gamut_matrix_filename;
	CString cct_matrix_filename;
	CString max_lv_filename;
	
	RGBColor cct_trans_lut[LUT_array_size];
	RGBColor cct_trans_trc_lut[LUT_array_size];

	vector<double> max_lv;
	MATRIX matrix_native_RGB_XYZ;
	MATRIX matrix_gamut_transform;
	MATRIX matrix_cct_transform;
	...
	fout.Write("W,R,G,B\n", 8);//
	char str[255];
	sprintf_s(str, "%f,", tone_curve.w);
	fout.Write(str, strlen(str));
	sprintf_s(str, "%f,", tone_curve.r);
	fout.Write(str, strlen(str));
	sprintf_s(str, "%f,", tone_curve.g);
	fout.Write(str, strlen(str));
	sprintf_s(str, "%f\n", tone_curve.b);
	fout.Write(str, strlen(str));
	fout.Close();
	tone_curve_raw = tone_curve.w;
	Update_Panel_1d_Lut_TRC(tone_curve_raw);
	Write_PIPE_GAMMA_LUT(tone_curve_raw);
	UpdateLut2Mcu_FPGA(SELECT_1D_LUT_DEGAMMA);
	Update_Panel_1d_Lut_TRC(1.0 / tone_curve_raw);
	Write_PIPE_GAMMA_LUT(1.0 / tone_curve_raw);
	UpdateLut2Mcu_FPGA(SELECT_1D_LUT_DEGAMMA_TEST);

	for (int i = 0; i < gamut_list.size(); i++) {
		if (i != 0) { Init_Panel_Matrix_1D_LUT_GAMMA(); }
		gamut_target = gamut_list[i];
		matrix_native_RGB_XYZ = Build_Matrix_RGB_to_XYZ(XYZ_R, XYZ_G, XYZ_B, XYZ_0, XYZ_W[1]);
		//gamut matrix
		if (gamut_target == "Native") {
			matrix_gamut_transform = matrix_raw;
		}
		else {
			matrix_gamut_transform = Build_Matrix_RGB_to_Gamut_RGB(matrix_native_RGB_XYZ, gamut_target);
		}
		Update_Gamut_Matrix(matrix_gamut_transform);
		UpdateMatrixRegisters();
		gamut_matrix_filename.Format(".\\data\\matrix\\gamut\\matrix_%s_gamut.csv", gamut_target);
		Write_Matrix(gamut_matrix_filename, matrix_gamut_transform);

		//cct calibrate to 6500K
		cct_target = 6500;
		OnBnClickedButtonTrc();
		CCT_Calibration();
		Update_Panel_1d_Lut(ca_lut);
		UpdateLut2Mcu_FPGA(SELECT_1D_LUT_GAMMA);
		gamut_65_lut_filename.Format(".\\data\\lut\\1d_lut\\lut_%s_6500.csv", gamut_target);
		Write_LUT(gamut_65_lut_filename, ca_lut);
		...
		for (int j = 0; j < cct_list.size(); j++) {
			...
			if (cct_target == 6500) {
				matrix_cct_transform = matrix_raw;
			}
			else {
				matrix_cct_transform = Build_Matrix_RGB_to_CCT_RGB(matrix_native_RGB_XYZ, cct_target);
			}
			CCT_Trans_LUT(matrix_cct_transform, &ca_lut[0], &cct_trans_lut[0]);
			max_lv = Measure_MaxLv(cct_trans_lut[LUT_array_size - 1]);
			cct_matrix_filename.Format(".\\data\\matrix\\cct\\matrix_%s_%d.csv", gamut_target, cct_target);
			Write_Matrix(cct_matrix_filename, matrix_cct_transform);
			...
			for (int k = 0; k < trc_list.size()-1; k++) {
				tone_curve.target = trc_list[k];
				TRC_Trans_LUT(cct_trans_lut, &cct_trans_trc_lut[0]);
				Update_Panel_1d_Lut(cct_trans_trc_lut);
				gamut_65_lut_filename.Format(".\\data\\lut\\all_lut_verify\\%s\\lut_%s_%d_%.2f.csv", gamut_target, gamut_target, cct_target, tone_curve.target);
				Write_LUT(gamut_65_lut_filename, cct_trans_trc_lut);
			}

			Cal_DICOM_LUT(cct_trans_lut, max_lv[1]);
			gamut_65_lut_filename.Format(".\\data\\lut\\all_lut_verify\\%s\\lut_%s_%d_dicom.csv", gamut_target, gamut_target, cct_target);
			Write_LUT(gamut_65_lut_filename, dicom_lut);
			Update_Panel_1d_Lut(dicom_lut);
		}
	}
	verify_data_updated = false;
...
void CCa200SampleDlg:: Colour_Verify() {
	...
	string str;
	bool gamut_load;
	if (!verify_data_updated) {
		Set_PIPE_Gamma_LUT();
		verify_data_updated = true;
	}
	for (int i = 0; i < gamut_list.size(); i++) {
		gamut_target = gamut_list[i];
		select = *select_list[i];
		...
		for (int j = 0; j < cct_list.size(); j++) {
			...
			for (int k = 0; k < trc_list.size(); k++) {
				...
				if (select[str]) {
					if (!gamut_load) {
						cstr_matrix.Format(".\\data\\matrix\\gamut\\matrix_%s_gamut.csv", gamut_list[i]);
						Update_Gamut_Matrix(Load_Matrix(cstr_matrix));
						UpdateMatrixRegisters();
						gamut_load = true;
					}
					if (k == trc_list.size() - 1) {
					...
					Meaure_Color(gamut_list[i], cct_list[j], trc_list[k]);
				}
			}
		}
	}
...
void CCa200SampleDlg::OnBnClickedButtonSetPipeline()
{
	...
	gamut_target = cstr_gamut;
	cstr_gamut_m.Format(".\\data\\matrix\\gamut\\matrix_%s_gamut.csv", cstr_gamut);
	Update_Gamut_Matrix(Load_Matrix(cstr_gamut_m));
	UpdateMatrixRegisters();
	if (cstr_trc == "DICOM") {
		cstr_lut.Format(".\\data\\lut\\all_lut_verify\\%s\\lut_%s_%d_dicom.csv", cstr_gamut, cstr_gamut, atoi(cstr_cct));
	}
	else
	{
		cstr_lut.Format(".\\data\\lut\\all_lut_verify\\%s\\lut_%s_%d_%.2f.csv", cstr_gamut, cstr_gamut, atoi(cstr_cct), atof(cstr_trc));
	}
...
void CCa200SampleDlg::Set_PIPE_Gamma_LUT() {
	CString str;
	str.Format(".\\data\\panel_attribute\\de_gamma.csv");
	Load_Panel_1D_LUT(str);
	UpdateLut2Mcu_FPGA(SELECT_1D_LUT_DEGAMMA);
	str.Format(".\\data\\panel_attribute\\en_gamma.csv");
	Load_Panel_1D_LUT(str);
	UpdateLut2Mcu_FPGA(SELECT_1D_LUT_DEGAMMA_TEST);
}
...
void CCa200SampleDlg::OnCbnSelchangeComboMode()
{
	...
	Generate_RGB_Pattern(LinkNo, one_d_lut[index].Red, one_d_lut[index].Green, one_d_lut[index].Blue);
	if (mode == "Verify") {
		Verify_Btn_Enable(1);
	}
	else {
...
{
	((CWnd*)GetDlgItem(IDC_CHECK_SRGB_18))->EnableWindow(0);
	Check_State(); //check link state
...
void CCa200SampleDlg::OnBnClickedCheckImage()
{
	...
	if (mode) {
		Generate_RGB_Pattern(LinkNo, one_d_lut[0].Red, one_d_lut[0].Green, one_d_lut[0].Blue, ACTION_TX_TEST_PATTERN_INSERTION);
	}
	else {
		int index = LUT_array_size - 1;
		Generate_RGB_Pattern(LinkNo, one_d_lut[index].Red, one_d_lut[index].Green, one_d_lut[index].Blue);
