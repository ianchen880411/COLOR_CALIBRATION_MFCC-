...
// Construction 
public:
	RGBColor panel_1d_lut[LUT_array_size];
...
protected:
	...
	vector<double> trc_list{ 1.8, 2.0, 2.2, 2.35, 2.4, 2.6, 0.00 };
	...
	void CCT_Trans_LUT(MATRIX matrix_cct, PRGBColor ptr_ca_lut, PRGBColor ptr_cct_lut);
	void TRC_Trans_LUT(RGBColor lut[LUT_array_size], PRGBColor ptr);

	void Update_Panel_1d_Lut(RGBColor lut[]);
	void Update_Panel_1d_Lut_TRC(double gamma);
	...
	void Meaure_Color(CString gamut, int cct, double trc);
	void CCT_Calibration();
	void Colour_Calibrate();
	bool verify_data_updated = false;
	...
	void Write_PIPE_GAMMA_LUT(double gamma);
