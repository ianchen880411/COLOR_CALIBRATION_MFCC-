...
bool CSerialPort::Open(LPCSTR lpszPortNum,
	...
	BYTE   byByteSize)
  {
...
#if 0
	timeouts.ReadIntervalTimeout = MAXDWORD; /* Maximum time between read chars. Not supported in Windows */
	...
	timeouts.WriteTotalTimeoutMultiplier = 0;  /* Multiplier of characters.        */
	timeouts.WriteTotalTimeoutConstant = 5000; /* Constant in milliseconds.        */
#else
	timeouts.ReadIntervalTimeout = 1; /* Maximum time between read chars. Not supported in Windows */
	timeouts.ReadTotalTimeoutMultiplier = 0; /* Multiplier of characters.        */
	timeouts.ReadTotalTimeoutConstant = 0; /* Constant in milliseconds.        */
	timeouts.WriteTotalTimeoutMultiplier = 0;  /* Multiplier of characters.        */
	timeouts.WriteTotalTimeoutConstant = 0; /* Constant in milliseconds.        */
#endif
	...
	DWORD  dwMaxWait)
{
	...
	if (!bSuccess)
	{
		...
		if (err == ERROR_IO_PENDING)
		{
			...
			else if (result == WAIT_TIMEOUT)
			{
				// Handle the error.
				CloseHandle(m_ov.hEvent);
				CSerialPort::ErrorToString("ReadData(): WaitForSingleObject() timeout");
				return WAIT_TIMEOUT;
			}
		}
	}
	...
	else
	{
		// Handle the error.
		CloseHandle(m_ov.hEvent);
		wsprintf(m_lpszErrorMessage, TEXT("Error ReadData(): No EV_RXCHAR occured\n"));
		return 0;
	}
