...
/* _NEW_ERROR_CODE_4_3DLUT_FLASH_SAVE_LOAD_ */
	case 71:
		nError = ERROR_MCU_3D_LUT_INVALID_ENTRY_INDEX;
		break;

	case 79:
		nError = ERROR_MCU_3D_LUT_READBACK_NOT_IDENTICAL;
		break;
...
int16_t WINAPI Xfer_LUTable3D_U32Data(int32_t LinkNo, uint32_t dwLUTIndex, uint32_t dwRGBData)
{
	...
	if (dwLUTIndex >= MAX_3D_LUT_ENTRIES)
		return ERROR_SCALER_LUT_INVALID_INDEX;

	if ( (dwRGBData & 0xC0000000) != 0x00000000 )
		return ERROR_SCALER_3DLUT_INVALID_DATA;

	sprintf(strCmd, "J %X %X\r", dwLUTIndex, dwRGBData);
	...
	if (nRet == err_internal_success)
	{
		...
		if (nRet == 0) {
			...
			if (strncmp(rdBuf, "J OK", 4) == 0) {
				...
				nError = ERROR_SCALER_SPI_REG_ACCESS_FAILED;
#endif
				goto _Xfer_LUTable3D_Element_Exit;
			}
			else if (strncmp(rdBuf, "UNKNOWN", 7) == 0) {
				CmdAck = SERIAL_CMD_UNKNOWN;
				nError = ERROR_SCALER_SERIAL_UNKNOWN_COMMAND;
...
//	uint32_t dwMaskLUTBypass, dwLUTEnable;
	...
	case SELECT_1D_LUT_ENGAMMA:
		dwLUTTmep = (SELECT_1D_LUT_ENGAMMA << SHIFT_BITS_SELECT_LUT);
		break;
	...
	case SELECT_1D_LUT_CCT:
		dwLUTTmep = (SELECT_1D_LUT_CCT << SHIFT_BITS_SELECT_LUT);
		break;
...
#if defined(_FPGA_SUPPORT_1D_CCT_LUT_) && (_FPGA_SUPPORT_1D_CCT_LUT_ == 1)
	case SELECT_1D_LUT_CCT:
		wLUT = (SELECT_1D_LUT_CCT);
		break;
#endif /* _FPGA_SUPPORT_1D_CCT_LUT_ */

#if defined(_FPGA_SUPPORT_1D_EXTRA_LUT_) && (_FPGA_SUPPORT_1D_EXTRA_LUT_ == 1)
	case SELECT_1D_LUT_EXTRA:
		wLUT = (SELECT_1D_LUT_EXTRA);
		break;
...
int16_t WINAPI Save_LUTBuffer_to_Flash_Calibrate_Section(int32_t LinkNo, uint32_t dwFlashLUTIndex)
{
	...
	if (dwFlashLUTIndex == 0) {
		sprintf(strCmd, "Q 2 0\r");
	}
	else { /* if (dwFlashLUTIndex > 0) */
		wLUT = (dwFlashLUTIndex - 1);
		sprintf(strCmd, "Q 1 %X\r", wLUT);
	}
...
int16_t WINAPI Load_LUTBuffer_from_Flash_Calibrate_Section(int32_t LinkNo, uint32_t dwFlashLUTIndex)
{
	...
	if (dwFlashLUTIndex == 0) {
		sprintf(strCmd, "Q 6 0\r");
	}
	else { /* if (dwFlashLUTIndex > 0) */
		wLUT = (dwFlashLUTIndex - 1);
		sprintf(strCmd, "Q 5 %X\r", wLUT);
	}
...
#if defined(_FPGA_SUPPORT_1D_CCT_LUT_) && (_FPGA_SUPPORT_1D_CCT_LUT_ == 1)
	case SELECT_1D_LUT_CCT:
		dwMaskLUTBypass = TX_VIDEO_PIPELINE_1D_LUT_CCT_BYPASS_MASK;
		dwLUTEnable = TX_VIDEO_PIPELINE_1D_LUT_CCT_ENABLE;
		break;
...
#if defined(_SUPPORT_FPGA_DATECODE_1206_ABOVE_) && ( _SUPPORT_FPGA_DATECODE_1206_ABOVE_== 1)
	case SELECT_1D_LUT_ENGAMMA:
		dwMaskLUTBypass = TX_VIDEO_PIPELINE_1D_LUT_ENGAMMA_DICOM_BYPASS_MASK;
		dwLUTEnable = TX_VIDEO_PIPELINE_1D_LUT_ENGAMMA_DICOM_ENABLE;
