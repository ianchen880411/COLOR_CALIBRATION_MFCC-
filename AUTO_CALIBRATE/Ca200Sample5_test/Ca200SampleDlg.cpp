...
void Write_LUT(CString filename, LUTable1D lut[]) {
	CFile fout;
	fout.Open(filename, CFile::modeCreate | CFile::modeWrite);
	...
	for (int i = 0; i < 1024; i++) {
		...
		sprintf_s(str, "%d,", lut[i].rgbColor.Red);
		fout.Write(str, strlen(str));//儲存R
		sprintf_s(str, "%d,", lut[i].rgbColor.Green);
...
void CCa200SampleDlg::Load_FPGA_1D_LUT(CString filename) {
	LUTable1D lut[LUT_array_size];
	MATRIX m; 
	RGBColor RGB;
	string s;
	s = filename;
...
void CCa200SampleDlg::Send_Get(vector<vector<int>> send_index, int16_t nRet, int32_t LinkNo, vector<vector<double> >& data) {
	...
	for (int i = 0; i < send_index.size(); i++) {
		...
		R = calibrate.one_d_lut[Ri].rgbColor.Red;
		G = calibrate.one_d_lut[Gi].rgbColor.Green;
		B = calibrate.one_d_lut[Bi].rgbColor.Blue;
...
void CCa200SampleDlg::CCT_LUT_TRC_Correction(const double tone_curve_target, const int interval) {
	vector<int> trc_correction_index;
	send_index.clear();

	for (int i = 0; i <= LUT_array_size; i += interval) {
		i = (i == LUT_array_size) ? (LUT_array_size - 1) : i;
	...
	calibrate.CCT_LUT_Tone_Curve_Trans(cct_lut, lv, tone_curve_target, interval);
}
...
void CCa200SampleDlg::Measure_Raw() {
	...
	calibrate.panel_tone_curve.r = calibrate.Calculate_TC(lv);
	lv.clear();
	...
	calibrate.panel_tone_curve.g = calibrate.Calculate_TC(lv);
	lv.clear();
	...
	calibrate.panel_tone_curve.b = calibrate.Calculate_TC(lv);
	lv.clear();
	...
	calibrate.panel_tone_curve.w = calibrate.Calculate_TC(lv);

	m_raw_trc_r.Format("%.2f", calibrate.panel_tone_curve.r);
	m_raw_trc_g.Format("%.2f", calibrate.panel_tone_curve.g);
	m_raw_trc_b.Format("%.2f", calibrate.panel_tone_curve.b);
	m_raw_trc_w.Format("%.2f", calibrate.panel_tone_curve.w);
...
void CCa200SampleDlg::Update_FPGA_LUT(PLUTable1D lut) {
...
void CCa200SampleDlg::Init_Panel_Matrix_1D_LUT_GAMMA() {
	LUTable1D gamma_lut[LUT_array_size];

	gamut_target = "Native";
	cct_target = 0;
	xy_str.Format("Initial...");
	SetDlgItemText(IDC_STATIC_xy, xy_str);
	Update_Gamut_Matrix(calibrate.mp_matrix_gamut_target["Native"]);
	UpdateMatrixRegisters();
	calibrate.Generate_Gamma_LUT(gamma_lut, 1.0, 1.0);
	Update_FPGA_LUT(gamma_lut);
...
void CCa200SampleDlg::XferLut2Mcu() {
	...
	for (LUT_index = 0; LUT_index < LUT_array_size; LUT_index++)
	{
		...
		SetDlgItemText(IDC_STATIC_xy, xy_str);
		nError = Xfer_LUTable1D_Element(LinkNo, LUT_index, &(fpga_lut[LUT_index]).rgbColor);
		if (nError != ERROR_SCALER_SUCCESS) {
			break;
		}
	}
...
void CCa200SampleDlg:: CCT_Calibration(LUTable1D LUT[]) {
	vector<float>xy = calibrate.CCT_to_xy(cct_target);
	targetx = xy[0];
	targety = xy[1];
	...
	send_index.clear();
	for (int k = 0; k < virtul_LUT_size; k++) {
		send_index.push_back({ (k + 1) * virtul_LUT_interval - 1,  (k + 1) * virtul_LUT_interval - 1 ,  (k + 1) * virtul_LUT_interval - 1 });
	}
	//cct
	int c = 0;
	while (true) {
		if (c == 20) { break; }
			...
			Send_Get(send_index, nRet, LinkNo, data);
			calibrate.Cal_CCT_LUT(&data, cct_target); //產生cct_lut
		}
		c++;
	}
	calibrate.Get_CCT_LUT(LUT);
	calibrate.Init_LUT();
}

void CCa200SampleDlg::Meaure_Color(const CString gamut, const int cct, const double trc) {
	Check_State();
	send_index.clear();
	CString matrix_filenake;
	CString str;

	if (trc == 0.0) {
		str.Format(".\\data\\lut\\trc\\lut_%f.csv", trc);
	}
	else {
		str.Format(".\\data\\lut\\trc\\lut_%s_%d_dicom.csv", gamut,cct);
	}

	Load_FPGA_1D_LUT(str);
	UpdateLut2Mcu_FPGA(SELECT_1D_LUT_GAMMA);

	matrix_filenake.Format(".\\data\\matrix\\matrix_%s_%d.csv", gamut, cct);
	Update_Gamut_Matrix(Load_Matrix(matrix_filenake));
	UpdateMatrixRegisters();
...
void CCa200SampleDlg:: Colour_Calibrate() {
	...
	CString trc_lutfilename;
	CString dicom_lut_filename;
	...
	if (calibrated) { Init_Panel_Matrix_1D_LUT_GAMMA(); }
	...
	fout.Write("W,R,G,B\n", 8);//
	char str[255];
	sprintf_s(str, "%f,", calibrate.panel_tone_curve.w);
	fout.Write(str, strlen(str));
	sprintf_s(str, "%f,", calibrate.panel_tone_curve.r);
	fout.Write(str, strlen(str));
	sprintf_s(str, "%f,", calibrate.panel_tone_curve.g);
	fout.Write(str, strlen(str));
	sprintf_s(str, "%f\n", calibrate.panel_tone_curve.b);
	fout.Write(str, strlen(str));
	fout.Close();

	cct_target = 6500;
	OnBnClickedButtonTrc();
	CCT_Calibration(cct_lut);
	Update_FPGA_LUT(cct_lut);
	UpdateLut2Mcu_FPGA(SELECT_1D_LUT_DEGAMMA_TEST);
	Measure_RGBW();

	CCT_LUT_TRC_Correction(1.0, 4);
	Update_FPGA_LUT(cct_lut);
	UpdateLut2Mcu_FPGA(SELECT_1D_LUT_DEGAMMA_TEST);
	OnBnClickedButtonTrc();
	Write_LUT(".\\data\\lut\\lut.csv", cct_lut);

	Measure_RGBW();
	matrix_native_RGB_XYZ = calibrate.Build_Matrix_RGB_to_XYZ(XYZ_R, XYZ_G, XYZ_B, XYZ_W, XYZ_0, XYZ_W[1]);
	
	for (int i = 1; i < gamut_list.size(); i++) {
		gamut_target = gamut_list[i];
		for (int j = 0; j < cct_list.size(); j++) {
			cct_target = cct_list[j];
			matrix_gamut_transform = calibrate.Build_Matrix_RGB_to_Gamut_RGB(matrix_native_RGB_XYZ, gamut_target, cct_target);
			Update_Gamut_Matrix(matrix_gamut_transform);
			UpdateMatrixRegisters();
			gamut_cct_matrix_filename.Format(".\\data\\matrix\\matrix_%s_%d.csv", gamut_target, cct_target);
			Write_Matrix(gamut_cct_matrix_filename, matrix_gamut_transform);
			...
			calibrate.Generate_DICOM_LUT(cct_lut, dicom_lut, max_lv[1], 1.0);
			dicom_lut_filename.Format(".\\data\\lut\\trc\\lut_%s_%d_dicom.csv", gamut_target, gamut_target, cct_target);
			Write_LUT(dicom_lut_filename, dicom_lut);
		}
	}
	
	double gamma_target;
	for (int k = 0; k < trc_list.size() - 1; k++) {
		gamma_target = trc_list[k];
		calibrate.Generate_Gamma_LUT(gamma_lut, gamma_target, 1);
		trc_lutfilename.Format(".\\data\\lut\\trc\\lut_%f.csv", gamma_target);
		Write_LUT(trc_lutfilename, gamma_lut);
	}
...
void CCa200SampleDlg:: Colour_Verify() {
	...
	Load_FPGA_1D_LUT(".\\data\\lut\\lut.csv");
	UpdateLut2Mcu_FPGA(SELECT_1D_LUT_DEGAMMA_TEST);
...
void CCa200SampleDlg::OnBnClickedButtonSetPipeline()
{
	...
	UpdateMatrixRegisters();
	if (cstr_trc == "DICOM") {
		cstr_lut.Format(".\\data\\lut\\all_lut_verify\\trc\\lut_%s_%d_dicom.csv", cstr_gamut, atoi(cstr_cct));
	}
	else
	{
		cstr_lut.Format(".\\data\\lut\\all_lut_verify\\trc\\lut_%s_%d_%.2f.csv", cstr_gamut, atoi(cstr_cct), atof(cstr_trc));
	}
	Load_FPGA_1D_LUT(cstr_lut);
	UpdateLut2Mcu_FPGA(SELECT_1D_LUT_GAMMA);
	Measure_Raw();
...
void CCa200SampleDlg::OnCbnSelchangeComboMode()
{
	...
	Generate_RGB_Pattern(LinkNo, calibrate.one_d_lut[index].rgbColor.Red, calibrate.one_d_lut[index].rgbColor.Green, calibrate.one_d_lut[index].rgbColor.Blue);
	if (mode == "Verify") {
		Verify_Btn_Enable(1);
	}
	else {
...
void CCa200SampleDlg::OnBnClickedCheckImage()
{
	...
	uint16_t R = calibrate.one_d_lut[0].rgbColor.Red;
	uint16_t G = calibrate.one_d_lut[0].rgbColor.Green;
	uint16_t B = calibrate.one_d_lut[0].rgbColor.Blue;

	if (mode) {
		Generate_RGB_Pattern(LinkNo, R, G, B, ACTION_TX_TEST_PATTERN_INSERTION);
	}
	else {
		int index = LUT_array_size - 1;
		Generate_RGB_Pattern(LinkNo, R, G, B);
	}
}
